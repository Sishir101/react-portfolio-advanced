...
function Education() {
  return (
    <section id="education" className="relative scroll-mt-24 bg-[#030712] py-16 sm:py-28">
        ...
        <div className="relative space-y-7 sm:space-y-10 before:absolute before:left-[19px] before:top-2 before:h-[calc(100%-1rem)] before:w-px before:bg-gradient-to-b before:from-cyan-400/50 before:to-purple-500/30 md:before:left-1/2 md:before:-translate-x-px">
          {education.map((item, index) => (
            <AnimateIn key={item.id} delay={index * 0.1}>
              <div
                className={`relative flex flex-col gap-5 sm:gap-6 md:flex-row md:items-start ${
                  index % 2 === 0 ? "md:flex-row-reverse" : ""
                }`}
              >
                <div className="hidden flex-1 md:block" />

                <div className="absolute left-0 flex h-9 w-9 sm:h-10 sm:w-10 items-center justify-center rounded-full border border-cyan-400/40 bg-[#030712] text-cyan-400 md:left-1/2 md:-translate-x-1/2 flex-shrink-0 text-base sm:text-lg">
                  <FaGraduationCap />
                </div>
