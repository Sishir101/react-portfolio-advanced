...
function Skills() {
  return (
    <section id="skills" className="relative scroll-mt-24 bg-[#030712] py-24 sm:py-28">
        ...
        <div className="grid gap-6 sm:gap-8 lg:grid-cols-2 xl:grid-cols-4">
          {skillCategories.map((category, catIndex) => (
            <AnimateIn key={category.title} delay={catIndex * 0.1}>
              <div className="h-full rounded-2xl border border-white/10 bg-white/5 p-6 sm:p-8 backdrop-blur-sm transition hover:border-cyan-400/20 hover:bg-white/[0.07]">
                <h3 className="mb-6 text-xl font-bold text-cyan-400">{category.title}</h3>
                      ...
                      <li key={skill.name}>
                        <div className="mb-2 flex items-center justify-between gap-3">
                          <span className="flex items-center gap-2 text-slate-200 min-w-0">
                            <Icon className="text-lg text-cyan-400 flex-shrink-0" />
                          ...
                          <span className="text-sm text-slate-500 flex-shrink-0">{skill.level}%</span>
                        </div>
